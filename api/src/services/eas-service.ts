...
async function generateAttestation(newsletterOwner: string, nonce: string, recipient: string, months: number): Promise<string> {
    ...
    const provider = new ethers.providers.JsonRpcProvider(process.env.URL_ALCHEMY_PROVIDER!)
...
async function getActivedSubscriptors(newsletterOwner: string, newsletterNonce: string): Promise<Array<string>> {
    ...
    const variables = {
        "where": {
            ...
            "attester": {
                "in": [new ethers.Wallet(process.env.WALLET_KEY!).address, process.env.NEWSLETTER_CONTRACT!]
            },
            "decodedDataJson": {
    ...
    const data = await request<AttestationsActivedResponseType>(process.env.EAS_API!, query, variables);
...
function fillSchemaJsonDecode(newsletterOwner: string, newsletterNonce: string) {
